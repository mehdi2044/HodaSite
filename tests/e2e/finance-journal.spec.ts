...
for (const locale of ["fa", "tr", "en"] as const) {
  test(`${locale}: accountant reviews, posts and reverses an exact journal on mobile`, async ({
    ...
    try {
      ...
      await expect(list.getByRole("heading", { name: t.title })).toBeVisible();
      ...
      await draft.nth(0).getByLabel(t.debit, { exact: true }).fill("100.0001");
      ...
      await draft.nth(1).getByLabel(t.credit, { exact: true }).fill("100.0001");
