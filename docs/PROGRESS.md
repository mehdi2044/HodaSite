...
- «جایگزینی فایل» (replace) یک مورد از شرح فاز بود که در این نسخه پیاده نشد؛ فعلاً باید حذف/آپلود دوباره کرد. زیر «سؤال‌ها برای مدیر پروژه».
- `S3Storage` روی MinIO سرویس موجود در `docker-compose.dev.yml` هنوز جداگانه با یک چرخهٔ آپلود/variant/stream کامل تست نشده — پیاده‌سازی کد فرقی با `LocalStorage` ندارد ولی مسیر واقعی MinIO راستی‌آزمایی صریح نشده. بک‌لاگ.
- Lighthouse برای «تصاویر با اندازهٔ درست» روی هدر فروشگاه دستی چک نشده؛ فقط ساختار `<picture>`/`srcset`/`width`/`height` درست است.
- درگ‌اند‌دراپ/مودال انتخاب رسانه به‌صورت تعاملی با ماوس در مرورگر تست نشده (نبود ابزار مرورگر در این نشست)؛ ولی کل مسیر واقعی («آپلود → PROCESSING → job واقعی sharp → READY با variant») با آپلود واقعی HTTP و ورود ادمین واقعی روی Docker واقعی تأیید شد — پایین را ببینید.
...
#### سؤال‌ها برای مدیر پروژه
- شرح فاز «جایگزینی فایل (replace) — Media جدید بسازد، ارجاعات همان id قبلی را نگه دارد» خواسته بود؛ چون این یعنی همان `id` باید محتوای جدید داشته باشد (نه رکورد جدید)، با معماری «صف پردازش async روی رکورد» کمی تناقض دارد (رکورد باید status را به PROCESSING برگرداند وقتی هنوز جاهای دیگر به آن ارجاع می‌دهند). فعلاً پیاده نشد؛ اگر لازم است، پیشنهاد: دکمهٔ «جایگزینی» = آپلود Media نو + به‌روزرسانی folderId/tags/altI18n قبلی روی رکورد جدید، سپس حذف‌نرم قدیمی (بدون تغییر id) — نیاز به تأیید قبل از پیاده‌سازی در فاز بعد یا اصلاحیه.
