...
- شکل JSON فیلد `colors` در `ThemeSettings` از یک شیء تخت به `{light:{...}, dark:{...}}` تغییر کرد (بدون مایگریشن، چون Json است) تا حالت تیره ممکن شود. Seed و همهٔ خواننده‌ها به‌روز شدند؛ اگر جای دیگری (خارج از این فاز) به شکل قدیمی وابسته بود، نیاز به هماهنگی دارد.
