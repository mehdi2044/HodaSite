...
## ادامهٔ طراحی با عمق ملایم — ۲۰۲۶-۰۹-۱۹

Implemented against docs v1.2 / D09, D10, D26, D31, D50, D63, D67.

- مالک پس از مشاهدهٔ نسخهٔ محلی، هدر و Hero لایه‌دار PR #44 را تأیید کرد و ادامهٔ طراحی خلاقانه با حس سه‌بعدی را خواست. این بخش همان بالای صفحه را حفظ می‌کند؛ توصیف قدیمی «چهار کارت هم‌اندازه» در گزارش پایین مربوط به PR #43 است و با PR #44 جایگزین شده است.
- دسته‌بندی‌های پایین خانه قاب برجسته، عنوان شناور و چیدمان متناوب دسکتاپ دارند. محصولات در موبایل دو ستونه‌اند؛ عکس و قیمت خوانا و دکمهٔ مستقل علاقه‌مندی روی عکس است. نوار خدمات از همان متن‌های قابل ویرایش ادمین استفاده می‌کند.
- عمق و زاویهٔ ملایم صرفاً CSS است؛ فقط برای ماوس دقیق و کاربری که کاهش حرکت را فعال نکرده اجرا می‌شود. لمس و reduced-motion ثابت‌اند. مدل سه‌بعدی محصول، canvas، کتابخانهٔ انیمیشن، درخواست تصویر اضافه یا حلقهٔ JavaScript اضافه نشده است.
- داده، متن، عکس، رنگ و ترتیب دسته‌ها همچنان از تنظیمات فعلی می‌آید. migration، seed یا اجرای مجدد اسکریپت ارتقای نمونه برای این تغییر لازم نیست.
- تست دستی: خانه را در fa/tr/en باز کنید؛ چهار دسته و تازه‌ها را در موبایل ببینید؛ روی دسکتاپ ماوس/Tab را روی دسته‌ها ببرید؛ با فعال‌کردن کاهش حرکت باید زاویه‌گیری متوقف شود. علاقه‌مندی باید بدون ورود به محصول تغییر کند و عکس/نام محصول مسیر درست را باز کند.
- بررسی تصویری رندر واقعی کامپوننت‌ها در عرض‌های ۳۹۰ و ۱۲۸۰ انجام شد. آزمون مرورگرِ مسیر دسته، بازگشت، reduced-motion، علاقه‌مندی و محصول به هر سه زبان اضافه شد؛ شواهد CI روی head نهایی در PR ثبت می‌شود. آزمایش iPhone واقعی و اندازه‌گیری نهایی سرعت تولید همچنان برای نوبت پذیرش بعدی باقی است.

## طراحی چهارگروهی بر پایهٔ مرجع لاویبل — ۲۰۲۶-۰۹-۱۹

Implemented against docs v1.2 / D09, D10, D26, D31, D54, D64, D67.

- کد و تصویر مرجع لاویبل بررسی شد؛ ترکیب قاب‌های لایه‌دار در Next.js واقعی پیاده شد. هیچ درخواست ساخت پولی به لاویبل ارسال نشد و runtime یا خرید نمایشی آن وارد پروژه نشد.
- چهار گروه زنانه، مردانه، بچه‌گانه و اکسسوری در Hero نمونه هم‌اندازه‌اند و نوار دسته‌ها در همهٔ صفحات مسیر مستقیم به کاتالوگ دارد. عنوان، عکس، ترتیب و لینک از دسته‌های دیتابیس می‌آید؛ برند و سه بازار/زبان حفظ شده‌اند. چیدمان تک‌تصویر/لایه‌دار از Homepage ادمین قابل انتخاب است.
- قاب مشترک ۴:۵ بدون زوم مصنوعی، کنترل fill/contain و نقطهٔ تمرکز با پیش‌نمایش در کتابخانهٔ رسانه، گالری بزرگ‌تر با دکمهٔ تصویر کامل، عکس در خلاصهٔ سبد و بستن آن پس از ناوبری اضافه شد. نشان فعال منوی پایین در مسیرهای canonical اصلاح شد؛ محصول لینک واضح بازگشت به دسته و لوگو لینک خانهٔ همان بازار دارد.
- مهاجرت D67 فقط ستون JSON نمایش رسانه را با پیش‌فرض خالی اضافه می‌کند. فایل اصلی، سفارش، قیمت، موجودی، مشتری و تنظیمات بازار بازنویسی نمی‌شوند. مرجع Lovable همچنان یک نمونهٔ جداست؛ این تحویل در مخزن HodaSite انجام می‌شود.
- فعال‌سازی روی دیتابیس نمونهٔ موجود: پس از migration، `scripts/upgrade-demo-storefront.ts` فقط ترکیب دست‌نخوردهٔ نمونهٔ قبلی را به layout جدید ارتقا می‌دهد؛ متن/تصویر سفارشی مالک و صفحه‌های اختصاصی بازار محفوظ می‌مانند. برای محتوای سفارشی، چیدمان جدید را از پنل انتخاب کنید.
- تست دستی: خانه در fa/tr/en، هر چهار دسته، بازگشت مرورگر، محصول، تصویر کامل/Escape، سبد و برند را در موبایل و دسکتاپ بررسی کنید. در ادمین یک تصویر را با contain و نقطهٔ تمرکز ذخیره کرده، صفحه را دوباره باز کنید. عکس‌های نمونه با برچسب مشخص‌اند و باید پیش از فروش با عکس واقعی محصولات جایگزین شوند.
- نتایج نهایی بررسی محلی و CI در PR ثبت می‌شود. iPhone واقعی، نصب/به‌روزرسانی HTTPS و سرعت نسخهٔ تولید هنوز پذیرش نشده‌اند. تأخیر گزارش‌شدهٔ محیط توسعه به‌تنهایی معیار سرعت نسخهٔ نهایی نیست؛ بررسی سنگین سرعت طبق خواستهٔ مالک به نوبت تست بعدی موکول است. فاز ۹ شروع نشده است.

## اصلاح مسیرهای نصب و به‌روزرسانی PWA — ۲۰۲۶-۰۹-۱۸

Implemented against docs v1.2 / D54, D64.

- گزارش بازبینی، ناسازگاری مسیرهای جدید زبان/بازار با تشخیص خانه در رابط نصب و سرویس‌ورکر را بازتولید کرد. قرارداد مشترک اکنون خانهٔ قدیمی و مسیرهایی مانند `/fa/m/IR`، `/tr/m/TR` و `/en/m/CA` را می‌پذیرد؛ محصول، سبد، پرداخت و مسیرهای خصوصی خانه محسوب نمی‌شوند.
- محافظ تک‌پنجره و تطبیق شناسهٔ درخواست‌کننده حفظ شده؛ وجود پنجرهٔ دیگر همچنان به‌روزرسانی را به تعویق می‌اندازد. تغییر worker نسخهٔ کش را خودکار عوض می‌کند.
- آزمون‌های مسیر، worker تولیدی و رابط نصب/به‌روزرسانی در سه زبان اضافه شدند. آزمون رابط از ثبت مصنوعی worker استفاده می‌کند؛ ادعای نصب واقعی نیست. نتیجهٔ CI نهایی در PR این اصلاح ثبت می‌شود.
- راهنمای فارسی اندروید شامل نصب از HTTPS، بررسی محلی با USB و چک‌لیست آزمون اضافه شد. هیچ تست گوشی، استقرار، اتصال واقعی AI یا پذیرش انتشار انجام‌شده تلقی نشده است. فاز ۹ شروع نشده است.

## اصلاح محلی کش Homepage و Header مشترک — ۲۰۲۶-۰۹-۱۶

Implemented against docs v1.2 / D09, D10, D26, D31, D54, D64.

- کش قدیمی Homepage با نسخهٔ جدید کلید کنار گذاشته شد. خواندن سبک `id/marketId/updatedAt` پیش از کش، تغییرات پنل، ساخت اولیه و upgrade از CLI را در درخواست بعدی منعکس می‌کند؛ محتوای بلوک‌ها همچنان کش می‌شود و `revalidateTag("homepage")` پنل حفظ شده است. تغییر شناسه/نسخه، ساخت override و حذف نرم، نتیجهٔ قدیمی را از دسترس خواندن خارج می‌کنند. migration یا endpoint جدید لازم نیست.
- Header مشترک در موبایل از سبد آیکون‌دار با تعداد و نام دسترس‌پذیر ترجمه‌شده استفاده می‌کند؛ نام برند تک‌خطی است. spacing موبایل برای عرض ۳۰۰ تنظیم شده؛ نمایش متنی سبد و ناوبری دسکتاپ حفظ می‌شوند. تنظیم Announcement Bar و بازارها تغییر نکرده است.
- آزمون کش با نتیجهٔ نگه‌داشته‌شده میان درخواست‌ها اضافه شد: تغییر سراسری پس از warm شدن سه بازار، ساخت اولیه، override/ویرایش/حذف و شکست خواندن نسخه. آزمون مرورگر جدید فقط‌خواندنی است و به fixtureهای دیتابیس دست نمی‌زند.
- راستی‌آزمایی محلی: lint کامل، lint نهایی فایل‌های مرتبط، typecheck و `git diff --check` موفق؛ ۳۴۸۲ آزمون واحد در ۷۴ فایل موفق؛ سه آزمون مرورگر شامل هر سه زبان در عرض‌های ۳۰۰/۳۲۰/۳۹۰ و دسکتاپ ۱۲۸۰ موفق. تصویر واقعی Hero، نام کامل و تک‌خطی برند، نام دسترس‌پذیر سبد، تعامل Enter، نبود overflow و جهت RTL/LTR بررسی شدند؛ تصاویر موبایل نیز خودبازبینی شدند. مجموعهٔ تغییردهندهٔ دیتابیس و CI اجرا نشدند. برای بارگذاری سورس جدید در bind mount ویندوز، app دوباره اجرا شد؛ رفع کش قدیمی پیش از آن بدون restart اثبات شده بود.
- تست دستی: `/fa/m/IR`، `/tr/m/TR` و `/en/m/CA` را در عرض ۳۰۰، ۳۲۰ و ۳۹۰ باز کنید؛ تصویر Hero، نام تک‌خطی برند، سبد فشرده، نبود overflow و جهت متن را بررسی کنید. سبد با Enter باز/بسته شود؛ در عرض ۱۲۸۰، متن سبد و منوی دسکتاپ دیده شوند.
- دادهٔ موجود reset یا seed نشده؛ تغییر مالی/موجودی/بکاپ وجود ندارد. به درخواست جدید مالک، این اصلاح برای تحویل از طریق branch و PR آماده شده است؛ نتیجهٔ CI روی head نهایی در PR ثبت می‌شود. مرج فعلاً مجاز نیست و فاز ۹ آغاز نشده است.

### پیگیری E2E در PR #40 — ۲۰۲۶-۰۹-۱۶

- اجرای CI شمارهٔ `35144649842`: هر دو job مربوط به Docker موفق؛ lint، typecheck، ۸۱۰۳ آزمون واحد/دیتابیس با coverage، build تولید و آزمون‌های بکاپ موفق؛ E2E با ۱۲۴ موفق و ۶ ناموفق پایان یافت.
- لاگ و artifact مرورگر دو ناسازگاری تست را اثبات کردند: سه تست خرید هنوز متن کامل سبد را در موبایل visible می‌خواستند، در حالی که طرح جدید عمداً آیکون + شمارنده دارد؛ سه تست Header نیز نام متنی برند را فرض می‌کردند، اما `admin-media.spec.ts` پیش‌تر لوگوی تصویری را از پنل ذخیره می‌کند و fixture آن باقی می‌ماند. trace و تصویر، لوگوی تصویری و شمارندهٔ درست سبد را نشان دادند؛ این شکست‌ها شاهد regression عملکرد فروشگاه یا کش نیستند.
- اصلاح محدود به تست‌ها: تست خرید کنترل قابل‌مشاهده، نام دسترس‌پذیر ترجمه‌شده و شمارندهٔ واقعی ۱ را بررسی می‌کند. تست Header در حالت متن همان شرط تک‌خطی/بدون برش را حفظ می‌کند و در حالت لوگو، تصویر بارگذاری‌شده، alt و جاگرفتن آن در محدودهٔ برند را بررسی می‌کند؛ نبود هر دو حالت خطاست. هیچ assertion مربوط به Hero، RTL/LTR، overflow، تعامل سبد یا دسکتاپ حذف نشده است.
- lint فایل‌های مرتبط، typecheck و سه تست Header در Local (هر سه زبان × عرض‌های ۳۰۰/۳۲۰/۳۹۰ و دسکتاپ ۱۲۸۰، حالت نام متنی) موفق شدند. اجرای محلی مسیر خرید که داده می‌سازد انجام نمی‌شود؛ اثبات کامل آن و حالت لوگوی تصویری به دیتابیس ایزولهٔ CI سپرده شده است. نتیجهٔ اجرای جدید و head دقیق در PR ثبت می‌شود؛ بدون reset دادهٔ Local، migration، تغییر کد محصول یا شروع فاز ۹. بازبینی انجام‌شده خودبازبینی و خودکار است، نه بازبینی مستقل انسانی؛ PR برای review وی‌بانو باز می‌ماند و مرج نمی‌شود.

### نتیجهٔ نهایی PR #40 — همگام‌سازی گزارش در ۲۰۲۶-۰۹-۱۸

PR #40 در ۲۰۲۶-۰۹-۱۶ با کامیت `f9c729bbf1434566df00d784508c1f194dab10b0` ادغام شد. روی head نهایی `dd93ffc8f47fce36f88e6b521a25e42e1ea5dd54`، [CI #204](https://github.com/mehdi2044/HodaSite/actions/runs/35147814439) هر سه job الزامی را گذراند: ۸۱۰۳ آزمون واحد/دیتابیس در ۱۱۷ فایل و ۱۳۰ آزمون مرورگر موفق. متن‌های «منتظر بررسی/CI» و «مرج نمی‌شود» در دو بند تاریخی بالا، وضعیت هنگام تهیهٔ آن گزارش‌ها هستند؛ PR اکنون باز نیست. این شواهد مربوط به PR40 است و جای آزمون اصلاح جدید PWA را نمی‌گیرد.

## فاز ۸ — تکمیل توسعه و ممیزی فازهای قبلی

شرح تغییرات، موارد اصلاح‌شدهٔ فازهای قبل، برنامهٔ تست دستی، اثر مهاجرت بر داده و موارد باز واقعی در [گزارش ممیزی](PHASE08_COMPLETION_AUDIT.md) آمده است. D65 پیش از تغییر schema ثبت شد. 08b در PR38 با سه job سبز، ۸۰۴۸ آزمون واحد/دیتابیس و ۱۱۷ آزمون مرورگر ادغام شده بود؛ عبارت قبلی «در حال تکمیل» مربوط به همان تاریخچه بود.

این تغییر نظرات/عکس/بررسی مدیر، wishlist، بازدید اخیر، اعلان موجودی، رضایت و analytics، تاریخچهٔ slug و 301، JSON-LD و تصویر اجتماعی، شواهد انتشار، cursor، قیمت‌گذاری batch، هشدار کنتراست و axe را پوشش می‌دهد. بکاپ دستی و بارگذاری/بازیابی در `/admin/system/backups` از فاز ۵ موجود است و حفظ می‌شود.

بیلد محلی analyzer: shared JS 103 kB، خانه/wishlist حدود 120 kB، دسته 121 kB، محصول 124 kB. گزارش head نهایی در artifact `bundle-analysis` CI ثبت می‌شود. این اعداد معیارهای واقعی موبایل را تأیید نمی‌کنند. اجرای واحد محلی: ۳۴۵۸ موفق؛ دیتابیس واقعی و آزمون مرورگر/restore در CI لازم‌اند. معیار ادغام همان موفقیت `checks`, `docker`, `docker-runtime` روی آخرین head است.
...
- D64 قبل از پیاده‌سازی ثبت شد: مسیر عمومی زبان/بازار مستقل از کوکی؛ canonical و hreflang از مبدأ HTTPS تنظیم‌شده و ترجمه‌های واقعاً موجود ساخته می‌شوند. لینک بازار صریح در مسیر قدیمی به مسیر پایدار منتقل می‌شود؛ انتخاب دستی بازار، کد بازار مقصد را در آدرس حفظ می‌کند؛ اگر زبان فعلی در مقصد فعال نباشد، صفحهٔ خانه با زبان پیش‌فرض مقصد باز می‌شود. فیلتر تکی در canonical حفظ می‌شود و ترکیب چند فیلتر noindex است.
...
- CI نخست، نبود تابع headers در شبیه‌ساز درخواست تست‌های خرید را آشکار کرد؛ شبیه‌ساز مسیر checkout با هدر خالی تکمیل شد. هیچ assertion مالی/موجودی حذف یا ضعیف نشده است؛ اجرای دوبارهٔ کامل الزامی است.

- آزمون مرورگر، تبدیل loopback در NextURL را آشکار کرد: rewrite به میزبان متفاوت هدر بازار را از دست می‌داد. استفاده از URL اصلی درخواست در rewrite/redirect و چهار آزمون واقعی NextRequest/NextResponse اضافه شد؛ میزبان و بازار PWA حفظ می‌شوند. نتیجهٔ مرورگر نسخهٔ اصلاح‌شده باید در CI نهایی تأیید شود.
- دو یافتهٔ بازبینی خودکار دربارهٔ حفظ بازار پس از تعویض و حفظ فیلتر تکی اصلاح شدند؛ ۱۰۱ آزمون مرتبط محلی موفق شدند و آزمون مرورگر حذف کوکی/بازگشایی آدرس اضافه شد.
...
- **کندی اولین اجرای مسیرها در توسعهٔ محلی:** مشکل اجرای موازی فایل‌های E2E رفع شده است؛ `playwright.config.ts` اکنون در همهٔ محیط‌ها `workers: 1` دارد. کامپایل اولیهٔ مسیرها در `next dev` همچنان ممکن است تأخیر داشته باشد؛ CI نسخهٔ buildشده را اجرا می‌کند. اجرای موازی محلی دیگر کار باز محسوب نمی‌شود.
