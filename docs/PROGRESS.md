...
- **محلی اجرا شد و سبز:** `prisma validate`, `prisma format --check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (۱۲/۱۲)، `docker compose ... config` برای هر دو فایل. Smoke با `next dev`: ریدایرکت‌های `/admin`، سرو فایل‌های فونت، endpointهای maintenance/state و ۴۰۱‌ها.
- **فقط در CI اجرا شد و سبز:** `pnpm prisma migrate deploy && pnpm prisma db seed`، `pnpm build` کامل، `bash scripts/backup/tests/run.sh`، `pnpm e2e` (۱۴ تست شامل ورود owner، آپلود/magic-byte، گیت تعمیرات). و در job جدید `docker`: `docker build --target runner .`، `docker build --target ops .` + بررسی `schema.prisma` و Prisma CLI در image ops، و `docker compose config`.
- **هنوز اصلاً راستی‌آزمایی نشده:** `docker compose -f docker-compose.yml up` کامل (اجرای هم‌زمان postgres/minio/migrate/app/caddy)، `docker compose run --rm ops npx prisma migrate deploy` روی دیتابیس واقعی، `restore.sh`/`backup.sh` داخل کانتینر `ops`. این ماشین Docker daemon و Postgres ندارد؛ باید روی سرور یا محیط دارای Docker اجرا شود.
- **بدهی شناخته‌شده:** `.env.example` کامنت‌های `KEY=value   # …` دارد که Docker Compose جزو مقدار می‌خواند (مثلاً `BACKUP_OFFSITE_ENDPOINT` مقدارش `# e.g…` می‌شود). خارج از A5–A7؛ در B9 یا یک اصلاح جدا باید کامنت‌های درون‌خطی حذف شوند.
