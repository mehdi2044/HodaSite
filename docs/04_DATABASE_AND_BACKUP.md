...
### Phase 05 return and credit migration (D55)
`20260911110000_phase05_returns` is additive: frozen return policy/net merchandise amounts, optimistic request versions, return/refund association and the `CreditUse` payment reservation ledger. Existing rows remain; no data or media is deleted. New return history and credit identity fields reject destructive edits. Default admin/warehouse gain return processing; accountant gains return processing with its existing refund permission. No new refund grant is given to admin. Store credits use the order currency and never mutate its amount or FX snapshot. Exchange shipping/service fees currently default to zero; original fees are excluded from automatic merchandise reimbursement. Phase 06 must post compensating financial entries from these retained records. Backups include the new tables automatically through pg_dump; restore must run migrations before normal traffic resumes. Do not downgrade the schema after creating credit-use/return records; use the isolated ops safety-backup restore workflow.

### S3 generation restore (D56)

Phase 05 backup review migration adds `BackupSettings.minuteUtc` (default 30, constrained 0–59) and `Backup.localPrunedAt`. Previously pruned rows recorded as failures are marked DONE with a separate pruning timestamp; their active controls and server operations remain unavailable. Apply migrations before the matching app/ops release. Old archive formats remain supported; uploaded newer migrations are rejected with a translated compatibility reason. No destructive downgrade or removal of backup history is introduced.
In S3 mode, backup mirrors the active logical object set into a private temporary directory before archiving. Internal `_hoda_restore/` generations are excluded when reading the legacy root. Restore extracts and validates against the restored DB, uploads to a new UUID generation, downloads that generation again and compares its complete contents before activating it. `S3_PREFIX_FILE` (default `/data/s3-prefix`) is an atomic local pointer shared by app and ops; invalid content fails closed. Application storage methods resolve it for every operation, while logical Media keys and application URLs stay unchanged. The old generation and failed staging generations remain for recovery; automatic generation deletion is deliberately not introduced in this step. Operators must plan retention/space before production. S3 pre-signed URLs issued before a switch may remain valid for their existing 15-minute lifetime.

There is no schema migration in this storage change. Existing root objects remain readable with an absent/empty pointer. Deploy app and ops with the generation-aware code before running the first S3 restore. Do not downgrade to code that ignores this pointer after activation: recover through the retained safety backup and compatible ops code. Snapshot/restore SQL, project/migration checks, off-site credentials and maintenance handshake are preserved. The new CI runs the full backup → verify → restore cycle for both providers and kills a real S3 staging process before pointer activation to verify the previous generation remains intact; results must be recorded before this acceptance is marked complete.

### Restore maintenance handshake hardening (Phase 05)
No database/schema/archive migration is required. Deploy `restore.sh` and its new sibling `maintenance.sh` together in the ops image. The handshake parses exactly one JSON object and requires both maintenance `on` and numeric `inFlight: 0`. Unconfirmed drain returns exit 21 before the safety backup or any live restore; failure to confirm maintenance `off` after verification returns exit 41 rather than reporting success. Archive validation failures no longer toggle live maintenance. Failures after attempting maintenance keep it on and raise the existing restore alert. Existing archive formats, object generations, immutable data and off-site configuration are unchanged. The shell suite executes the real restore control flow with isolated command fixtures; Docker/real-provider acceptance must be rerun before merging these changes.

### Owner control panel and ops queue (D56)
`20260912010000_phase05_backup_panel` adds OpsTask, BackupUpload and BackupSettings. Backups include these tables automatically. App mounts `/backups` read-only and a separate quarantine volume read-write; ops has both writable. The worker runs under a shared-volume `flock`, never a transaction spanning pg_restore. Requests carry a fixed operation type and validated identifiers, not commands. Restore requires a current owner with backup.restore plus password and a newly consumed TOTP; the worker rechecks the owner/permission and a five-minute authorization expiry before execution.

Operation markers are stored outside the restored database before execution. After restore/restart, their terminal status recreates/reconciles task rows; interrupted work fails closed instead of replaying. Passwords, authenticator codes, subprocess output and secrets are not persisted in task payloads or shown in the browser. Uploaded ZIPs are restricted to generated root members, exact checksums and known migrations, then restored only to a scratch verification database before being eligible for the live restore flow.

Schedule/retention changes require the owner and backup.create. Save and enable the schedule explicitly; the UTC hour and weekday are displayed. Retention selects distinct newest daily/weekly/monthly scheduled snapshots. It does not delete manual/safety backups or remote mirrors. S3 generation retention remains separate. The ops container must be running for queued work, schedules and quarantine cleanup; a stopped worker leaves visible PENDING requests. First deploy must build both app and ops and apply the additive migration. Rollback uses compatible ops plus safety backup; do not manually drop operation tables.

### Phase 06 operations migration — D60
`20260914010000_phase06_operations` adds finance configuration, Supplier, PurchaseOrder/items, Expense, Partner and CapitalTransaction, their references, amount constraints and immutable final-history guards. Journal actors may be null for customer/system-triggered domain events; an automated action never impersonates an administrator. Existing manual journal actors remain unchanged. The migration inserts only missing chart accounts and never rewrites orders/payments/stock or enables recognition. Whole-database pg_dump includes all new records; the original archive validation and ops maintenance/safety-backup/restore/migrate sequence stays mandatory. No manual DROP/reset is part of this upgrade. Rollback must preserve new financial history; use compatible code and the established safety-recovery process.

Phase 06 attribution stores frozen classification and journal allocations separately from mutable product descriptions. Journal reversal creates compensating attribution records. Expense documents are ordinary storage objects included in the same database/media backup; their dedicated authenticated route does not create a new storage provider. The new migration is additive; no historical sale/expense is inferred or overwritten.
