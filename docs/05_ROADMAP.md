# نقشهٔ راه اجرایی — ۱۲ فاز اصلی، مرحلهٔ ۰۵b و ۵ Checkpoint
...
```
 ...
 Phase 05b طراحی موبایل فروشگاه و حساب مشتری + نصب‌پذیری PWA در Android/iPhone
 ...
 Phase 08  SEO فنی + تأیید نهایی PWA + Performance + Wishlist + نقد ساده + Accessibility + Launch Checklist
...
| 01c | CMS & Storefront Design | منوها، Pages/بلوک‌ها، Homepage Builder، Translation Editor، Notification Templates، طراحی نهایی 390px — پیاده‌سازی روی برنچ فاز کامل؛ منتظر CI/بازبینی | ۱–۲ روز |
...
| 05b | Mobile Storefront / PWA | طراحی یکپارچهٔ خانه، محصول، سبد، پرداخت و حساب در سه زبان؛ نصب اندروید/آیفون، آفلاین امن و به‌روزرسانی کش | پس از بررسی بصری مسیرهای موجود برآورد می‌شود |
...
| 08 | SEO/PWA/Perf | تأیید نهایی نصب روی گوشی، hreflang، sitemap، سرعت، wishlist، Launch Checklist | ۲–۳ روز |
...
## زمان‌بندی زیرساخت — D51
- توسعه و تست خودکار پیش از خرید هاست و دامنه ادامه می‌یابد.
- نزدیک انتشار: staging با HTTPS، ایمیل واقعی، بکاپ مستقل خارج از هاست و آزمون بازیابی واقعی آماده می‌شود. معیارهای دستی معوق CP1/CP2 حذف نشده‌اند.
- نصب، بازشدن مستقل، اجرای دوباره و به‌روزرسانی وب‌اپ روی Android و iPhone واقعی باید با HTTPS آزمایش شود؛ شبیه‌سازی موبایل جای آن را نمی‌گیرد.
- Soft launch فقط پس از تکمیل Launch Checklist، تست دسترسی سه کشور و مسیر کامل خرید انجام می‌شود.

## ترتیب تحویل به‌روز — D54/D57
۰۵ تکمیل عملیات و کنترل‌ها → **۰۵b طراحی موبایل و PWA** → تحویل داشبورد ۰۶ مالی → ۰۷ ورود اطلاعات AI → ۰۸ SEO و کیفیت نهایی انتشار. معیارهای نصب پایه از ۰۸ به ۰۵b منتقل شده‌اند؛ شماره‌های فازهای بعدی تغییر نمی‌کنند. طبق D57 آماده‌سازی محاسبات مالی می‌تواند هنگام بستن ۰۵ شروع شود؛ PR #20 همین بخش را آغاز کرده و به معنی فعال‌شدن دفتر مالی یا پایان ۰۵ نیست.
