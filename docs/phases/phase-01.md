...
6. **Pages (CMS)**: `Page` with slug per locale, type static/landing, status, SEO, block-based content. Blocks: RichText (dependency-free visual editor per D48, RTL-aware), Image, Hero, TwoColumns, FAQ, CTA, Countdown, Embed. Storefront route `/[locale]/pages/[slug]`. Seed pages: about, contact, terms, privacy, returns, size-guide, faq (placeholder text in 3 languages).
