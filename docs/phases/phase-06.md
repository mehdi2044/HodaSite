...
## Read-only order reconciliation delivery (2026-09-13)

Before automatic recognition, `/admin/finance/reconciliation` and its order detail compare financial evidence under the existing `finance.report.view` market permission. Inclusive UTC payment-date filters, a maximum 366-day period and explicit 30-order keyset pages preserve historical market visibility. Orders without `paidAt` are excluded from dated lists; direct authorized detail flags missing payment dates and quotes. Related records are read in a RepeatableRead snapshot. Contact, address, customer identifiers, bank/receipt details and payment references never enter the returned financial DTO.

The pure Decimal comparison separates items, discounts, charged/absorbed fees, external approved payments and consumed store credit. Credit evidence must match approved credit payments one to one; mutable credit balances are not evidence of historical consumption. Foreign currencies are flagged and never added to original-currency totals. Return/refund activity requires separate review; no net-after-return claim is made. Stored order quotes reproduce original TRY/USD equivalents directly, without current FX or a round-trip through rounded USD. A 12-decimal inverse-rate conversion that changes four-decimal equivalents is flagged rather than silently adjusted.

Matched means only that the recorded order and original settlement agree. It is not eligibility to auto-post, proof of bank balance, profit or historical cutover. There is no migration, new accounting policy, financial mutation, provider call or publication in this delivery. Automatic payment/refund posting, credit-liability accounts and explicit historical opening/cutover handling remain subsequent work. Tests cover monetary edge cases, real PostgreSQL role/scope and privacy/no-write checks, pagination, and three-language mobile Chromium/WebKit flows.
