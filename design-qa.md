...
- Full storefront/header and real database-backed navigation: pending CI browser captures. The temporary harness proves rendering and target hit areas only; its fixture links do not prove commerce.
...
4. **P2 — header density differed substantially.** Main navigation now contains root departments and utilities use a separate compact row. Full runtime capture is required before this finding can close.
